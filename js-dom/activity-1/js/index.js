...
rect.style.pointerEvents = 'none'; // mouse hovering is not interpreted
...
function renderCircle(coord, parent) {
    ...
    circle.style.fill = `rgb(0, ${coord.x}, ${coord.y})`; // no red no greed only blue 

    // Append
    parent.appendChild(circle);
}

// renderCircle({ x: 150, y: 200 }, svg);
...
drawCircles([{ x: 100, y: 60 }, { x: 150, y: 120 }, { x: 200, y: 200 }], svg);
...
function updateText(data) {
    ...
    p.innerHTML = `There are ${data.length} circles and the average color is rgb(0, ${green}, ${blue})`
}
